...
class TLSExtensionEncryptThenMAC(TLSExtensionSignature,
        ext_type=TLSExtensionType.encrypt_then_mac):
    def __init__(self, length):
        super().__init__(self.ext_type, length=length)

    @classmethod
    def from_dict(cls, d):
        return TLSExtensionEncryptThenMAC(**d)

    @classmethod
    def from_bytes(cls, length: int, data: bytes):
        return TLSExtensionEncryptThenMAC(length)
...
class TLSExtensionPostHandshakeAuth(TLSExtensionSignature,
        ext_type=TLSExtensionType.post_handshake_auth):
    def __init__(self, length):
        super().__init__(self.ext_type, length=length)

    @classmethod
    def from_dict(cls, d):
        return TLSExtensionPostHandshakeAuth(**d)

    @classmethod
    def from_bytes(cls, length: int, data: bytes):
        return TLSExtensionPostHandshakeAuth(length)
...
class TLSExtensionNextProtocolNegotiation(TLSExtensionSignature,
        ext_type=TLSExtensionType.next_protocol_negotiation):
    def __init__(self, length):
        super().__init__(self.ext_type, length=length)

    @classmethod
    def from_dict(cls, d):
        return TLSExtensionNextProtocolNegotiation(**d)

    @classmethod
    def from_bytes(cls, length: int, data: bytes):
        return TLSExtensionNextProtocolNegotiation(length)
...
class TLSClientHelloSignature(Signature):
    ...
    def from_bytes(cls, record: bytes):
        ...
        if (len(record) - off - 4 != 
            (handshake_header.length_high << 16) +
            handshake_header.length_low):
...
def process_pcap(pcap) -> List[bytes]:
    ...
    """
    client_hellos = []
    for ts, buf in dpkt.pcap.UniversalReader(pcap):
        eth = dpkt.ethernet.Ethernet(buf)
        if (not isinstance(eth.data, dpkt.ip.IP) and
                not isinstance(eth.data, dpkt.ip6.IP6)):
            continue
        ip = eth.data
